...
SFR(IE_EX,	0xE8);	// extend interrupt enable
   ...
   SBIT(IE_PWMX,	0xE8, 5);	// enable PWMX interrupt
   SBIT(IE_UART3,	0xE8, 5);	// enable UART3 interrupt
   SBIT(IE_UART1,	0xE8, 4);	// enable UART1 interrupt
   SBIT(IE_ADC,	0xE8, 3);	// enable ADC interrupt
   SBIT(IE_UART2,	0xE8, 3);	// enable UART2 interrupt
   SBIT(IE_USB,	0xE8, 2);	// enable USB interrupt
   SBIT(IE_INT3,	0xE8, 1);	// enable external interrupt INT3
...
SFR(P0,	0x80);	// port 0 input & output
   ...
   SBIT(P0_1,	0x80, 1);	
   SBIT(P0_0,	0x80, 0);	
   SBIT(TXD3,	0x80, 7);	// TXD output for UART3
   SBIT(AIN15,	0x80, 7);	// AIN15 for ADC
   SBIT(RXD3,	0x80, 6);	// RXD input for UART3
   SBIT(AIN14,	0x80, 6);	// AIN14 for ADC
   SBIT(TXD2,	0x80, 5);	// TXD output for UART2
   SBIT(AIN13,	0x80, 5);	// AIN13 for ADC
   SBIT(RXD2,	0x80, 4);	// RXD input for UART2
...
SFR(P2,	0xA0);	// port 2 input & output
   ...
   SBIT(P2_1,	0xA0, 1);	
   SBIT(P2_0,	0xA0, 0);	
   SBIT(PWM7,	0xA0, 7);	// PWM output for PWM7
   SBIT(TXD1,	0xA0, 7);	// TXD output for UART1
   SBIT(PWM6,	0xA0, 6);	// PWM output for PWM6
   SBIT(RXD1,	0xA0, 6);	// RXD input for UART1
   SBIT(PWM0,	0xA0, 5);	// PWM output for PWM0
   ...
   SBIT(PWM2,	0xA0, 3);	// PWM output for PWM2
   SBIT(PWM3,	0xA0, 2);	// PWM output for PWM3
   SBIT(INT0_,	0xA0, 2);	// alternate pin for INT0
   SBIT(PWM4,	0xA0, 1);	// PWM output for PWM4
   SBIT(PWM5,	0xA0, 0);	// PWM output for PWM5
...
SFR(P3,	0xB0);	// port 3 input & output
   ...
   SBIT(P3_1,	0xB0, 1);	
   SBIT(P3_0,	0xB0, 0);	
   SBIT(INT3,	0xB0, 7);	// external interrupt 3 input
   SBIT(CAP0,	0xB0, 6);	// capture0 input for timer2
...
SFR(P4,	0xC0);	// port 4 input & output
   ...
   SBIT(P4_1,	0xC0, 1);	
   SBIT(P4_0,	0xC0, 0);	
   SBIT(XO,	0xC0, 7);	// XO pin for external crystal oscillator
   SBIT(XI,	0xC0, 6);	// XI pin for external crystal oscillator
