...
__xdata __at (EP0_ADDR) uint8_t  Ep0Buffer[64];
...
__xdata __at (EP2_ADDR) uint8_t  Ep2Buffer[8];       //on page 47 of data sheet, the receive buffer need to be min(possible packet size+2,64)
__xdata __at (EP3_ADDR) uint8_t  Ep3Buffer[128];     //IN and OUT buffer, must be even address

#if (EP3_ADDR+128) > USER_USB_RAM
...
void USB_EP0_SETUP(){
    ...
    if(len == (sizeof(USB_SETUP_REQ)))
    {
        ...
        if ( ( UsbSetupBuf->bRequestType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )//Not standard request
        {
            ...
            {
                case USB_REQ_TYP_VENDOR:    //Digispark.c uses USB_TYPE_VENDOR, TINY ISP uses vendor too.
                {
                    switch( SetupReq )
                    {
                        case USBTINY_ECHO:
                        ...
                        case USBTINY_EEPROM_WRITE:
                            len = tinySpiSetupHandler();
                            usbMsgFlags = USB_FLG_USE_USER_RW;
                            break;
                        case 60: // debugWIRE transfer
                            usbMsgFlags= USB_FLG_USE_USER_RW;
                            len = debugWireSetupHandler();
                            break;
                        default:
...
void USB_EP0_OUT(){
    if ( usbMsgFlags & USB_FLG_USE_USER_RW )
    {
        switch(SetupReq)
        {
            ...
            default:
                UEP0_T_LEN = 0;
                UEP0_CTRL |= UEP_R_RES_ACK | UEP_T_RES_ACK;  //Respond Nak
                break;
        }
    }else{
...
void USBInterrupt(void) {   //inline not really working in multiple files in SDCC
    ...
    if(UIF_BUS_RST) {
        ...
        UEP2_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK;                //Endpoint 1 automatically flips the sync flag, and IN transaction returns NAK
...
void USBDeviceEndPointCfg()
{
    ...
    UEP2_3_MOD = 0xC4;                                                         //Endpoint2 double buffer
    UEP1_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK;                //Endpoint 1 automatically flips the sync flag, and IN transaction returns NAK
    UEP2_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK;                //Endpoint 2 automatically flips the sync flag, and IN transaction returns NAK
    UEP3_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK | UEP_R_RES_ACK;        //Endpoint 3 automatically flips the sync flag, IN transaction returns NAK, OUT returns ACK
