...
volatile __xdata uint8_t USBByteCountEP3 = 0;      //Bytes of received data on USB endpoint
...
uint8_t USBSerial_write(char c){  //3 bytes generic pointer
    ...
    if (controlLineState > 0) {
        while (true){
            waitWriteCount = 0;
            while (UpPoint3_Busy){//wait for 250ms or give up, on my mac it takes about 256us
                waitWriteCount++;
                delayMicroseconds(5);
...
uint8_t USBSerial_available(){
    return USBByteCountEP3;
}

char USBSerial_read(){
    if(USBByteCountEP3==0) return 0;
...
void USB_EP3_OUT(){
    if ( U_TOG_OK )                                                     // Discard unsynchronized packets
    {
        ...
        if (USBByteCountEP3)    UEP3_CTRL = UEP3_CTRL & ~ MASK_UEP_R_RES | UEP_R_RES_NAK;       //Respond NAK after a packet. Let main code change response after handling.
    }
}
